# C-039 / FR-035 — private_endpoint_enabled = true ⇒ public access disabled and
# exactly one private endpoint with subresource "searchService" registering into
# the supplied privatelink.search.windows.net zone.
...
variables {
  ...
  private_dns_zone_ids       = ["/subscriptions/00000000-0000-0000-0000-000000000001/resourceGroups/rg-dns-shd-hub-prd-swc-001/providers/Microsoft.Network/privateDnsZones/privatelink.search.windows.net"]
}
...
run "public_access_disabled" {
  ...
  assert {
    condition     = azurerm_search_service.this.public_network_access_enabled == false
    error_message = "private_endpoint_enabled=true must set public_network_access_enabled = false."
  }
}

run "one_private_endpoint_subresource_searchservice" {
  command = plan
  ...
  assert {
    condition     = one(azurerm_private_endpoint.this[*].private_service_connection[0].subresource_names[0]) == "searchService"
    error_message = "The search private endpoint subresource must be \"searchService\"."
  }

  assert {
    condition     = one(azurerm_private_endpoint.this[*].private_dns_zone_group[0].private_dns_zone_ids[0]) == var.private_dns_zone_ids[0]
    error_message = "The private endpoint must register into the supplied privatelink.search.windows.net zone."
  }
}
