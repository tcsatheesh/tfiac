...
# ----- C-014 (Amendment 2026-05-31) — Shared hub LA wiring (variable accepted for symmetry; no diagnostic setting emitted because Microsoft.ManagedIdentity/userAssignedIdentities exposes no diagnostic categories) -----
variable "shared_log_analytics_workspace_id" {
  description = "Shared hub Log Analytics workspace resource ID. Accepted for interface symmetry across wrappers; the underlying resource type emits no diagnostic categories so no azurerm_monitor_diagnostic_setting is wired. See spec.md C-014."
  type        = string
...
variable "diagnostic_settings_enabled" {
  description = "Accepted for interface symmetry; no-op for this wrapper (UAI has no diagnostic categories). See spec.md C-014."
  type        = bool
  default     = true
}
