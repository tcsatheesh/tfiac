...
# ----- C-013 (Amendment 2026-05-31) — APIM hub-only topology guard -----
variable "topology" {
  description = "Stack topology discriminator (`hub`|`spoke`). C-013 hard-fails unless this is `hub` (defence-in-depth at the wrapper boundary)."
  type        = string

  validation {
    condition     = contains(["hub", "spoke"], var.topology)
    error_message = "topology must be \"hub\" or \"spoke\" (spec.md CA-003)."
  }
}
