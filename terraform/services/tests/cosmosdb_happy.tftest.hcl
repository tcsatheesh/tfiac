# FR-032 — Cosmos DB happy path. Selecting a `cosmosdb` service (private-only)
# requires the vnet/dns remote-state backends; the stack resolves the PE subnet
# (development role) + the cosmos-sql zone id and wires them into
# module.cosmosdb, which emits exactly one private-by-default account + PE.
...
run "cosmosdb_pe_wired" {
  command = plan

  assert {
    condition     = local.cosmosdb_pe_subnet_id == "/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/rg-net-uc1-sp01-npd-swc-001/providers/Microsoft.Network/virtualNetworks/vnet-uc1-sp01-npd-swc-001/subnets/snet-dev-uc1-sp01-npd-swc-001"
    error_message = "FR-032: cosmosdb_pe_subnet_id must resolve from the vnet remote state for the development role."
  }

  assert {
    condition     = length(local.cosmosdb_pe_zone_ids) == 1
    error_message = "FR-032: cosmosdb_pe_zone_ids must resolve the single cosmos-sql zone id from the dns remote state."
  }

  assert {
    condition     = length([for k, v in module.cosmosdb : v if v != null]) == 1
    error_message = "FR-032: exactly one cosmosdb module instance must be created."
  }
}
