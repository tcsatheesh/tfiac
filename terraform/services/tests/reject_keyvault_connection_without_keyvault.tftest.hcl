# FR-045 / C-061 — enabling the Key Vault connection without a 'keyvault'
# selection hard-fails at plan time via
...
run "rejects_keyvault_connection_without_keyvault" {
  command = plan
  expect_failures = [
    check.aifoundry_keyvault_connection_prereqs,
  ]
