# C-018 / FR-027 — Foundry account private endpoint happy path.
# enable_aifoundry_private_endpoint = true with an `aifoundry` selection plus
# vnet/dns remote-state stubs resolves a subnet id + three zone ids and wires
# them into module.aifoundry, which emits exactly one private endpoint.
...
run "aifoundry_pe_wired" {
  command = plan

  assert {
    condition     = local.pe_subnet_id == "/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/rg-net-uc1-sp01-npd-swc-001/providers/Microsoft.Network/virtualNetworks/vnet-uc1-sp01-npd-swc-001/subnets/snet-dev-uc1-sp01-npd-swc-001"
    error_message = "C-018: pe_subnet_id must resolve from the vnet remote state for the development role."
  }

  assert {
    condition     = length(local.pe_zone_ids) == 3
    error_message = "C-018: pe_zone_ids must resolve all three (cogsvc/openai/aiservices) zone IDs from the dns remote state."
  }

  assert {
    condition     = length([for k, v in module.aifoundry : v if v != null]) == 1
    error_message = "C-018: exactly one aifoundry module instance must be created."
  }
}
