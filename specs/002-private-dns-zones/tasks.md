...
- [ ] T001 Create directory [modules/dnszones/](modules/dnszones/) with empty placeholder files `main.tf`, `variables.tf`, `outputs.tf`, `locals.tf`, `README.md`
- [ ] T002 Create directory [terraform/dns/tests/](terraform/dns/tests/) and [terraform/dns/tests/snapshots/](terraform/dns/tests/snapshots/) (will hold `.tftest.hcl` fixtures + reference.json)
- [ ] T003 [P] Capture the legacy state by running `terraform state list` (or equivalent) against the existing [terraform/dns/](terraform/dns/) backend and saving the inventory to `specs/002-private-dns-zones/legacy-state-inventory.txt` for the US4 `moved {}` block authoring. Do NOT delete the legacy files yet.
- [ ] T004 [P] In [terraform/dns/providers.tf](terraform/dns/providers.tf) (new file alongside the legacy one — keep both during the diff), draft the `terraform { required_version = "~> 1.9" required_providers { azurerm = { source = "hashicorp/azurerm", version = "~> 4.0" } } }` block + `provider "azurerm" { features {} subscription_id = var.subscription_id }`. File will replace the legacy `providers.tf` in T039.
- [ ] T005 [P] Create [variables/prd/hub/dns.tfvars](variables/prd/hub/dns.tfvars) with the reference input from [quickstart.md](quickstart.md) (`subscription_id`, `region = "swedencentral"`, `repo`, `topology = "hub"`, `tenant = "hub"`, `environment = "prd"`, `custom_zones = ["internal.contoso.local"]`, `disable_catalogue_zones = []`).
...
- [ ] T006 Extend [modules/naming/catalogue.tf](modules/naming/catalogue.tf) `local.services` with the `private_dns_zone` entry: `{ caf_abbr = "pdnsz", shape = "hyphenated", topology_scope = "prd-hub-only", category = "top-level", max_length = 63, charset = "alphanumeric-hyphen", case_rule = "lowercase", must_start_with_letter = true, child_keys = [] }` per [research.md § 1](research.md).
- [ ] T007 Extend [modules/naming/catalogue.tf](modules/naming/catalogue.tf) `local.defaults` with `private_dns_zone = { soa_record_email = null }` per [research.md § 2](research.md). Ensures `check.catalogue_completeness_defaults` continues to pass.
- [ ] T008 Run `cd modules/naming && terraform fmt -recursive && terraform validate && terraform test` to confirm the engine extension does not regress the 40 existing fixtures.
- [ ] T009 Regenerate [modules/naming/tests/snapshots/reference.json](modules/naming/tests/snapshots/reference.json) using the same `terraform console` pattern as feature 001 (see [quickstart.md § Regenerating](quickstart.md#regenerating-the-determinism-snapshot)). Commit the new bytes.
- [ ] T010 [P] Author [modules/dnszones/variables.tf](modules/dnszones/variables.tf) with `naming` (object passthrough of `module.naming.names` — typed as `map(any)`), `region` (string), `region_code` (string — short engine-mapped code such as `sdc` for `swedencentral`, derived in the root stack from `local.region_codes` and passed in explicitly to avoid re-deriving it inside the module), `custom_zones` (`list(string)`, default `[]`), `disable_catalogue_zones` (`list(string)`, default `[]`), and `input` (the engine input object for tag derivation per [plan.md Constitution VIII note](plan.md#constitution-check)). Variable-level validations for the FQDN regex and de-dup on `var.custom_zones` and `var.disable_catalogue_zones` per [contracts/input-schema.md](contracts/input-schema.md).

  ADDITIONALLY, catalogue-aware validation for `var.disable_catalogue_zones`:

  - **As implemented (root-level `terraform_data` precondition)**: After two original attempts (variable-validation and module-precondition) hit Terraform 1.9 limitations — `variable.validation` cannot reach module-local `local.catalogue`, `expect_failures` cannot target child-module resources, and `check {}` only emits warnings — the guard is sited as `terraform_data.guard_disable_keys_known` in [terraform/dns/validate.tf](terraform/dns/validate.tf) with a `lifecycle.precondition {}` block. The precondition consumes `module.dnszones.catalogue_keys` (a sorted, read-only output) and emits an error message naming the offending key(s) and listing the valid catalogue keys. The negative test in T031 targets `expect_failures = [terraform_data.guard_disable_keys_known]`.

  Encapsulation contract: the catalogue MAP stays in [modules/dnszones/locals.tf](modules/dnszones/locals.tf) (single source of truth). The module exposes only two thin sorted lists — `catalogue_keys` (strings) and `catalogue_fqdns` (diagnostics-only, consumed by the shadowing guard from T027). Adding/removing catalogue zones remains a one-PR edit to the module. This deviation from the original "module-precondition" plan is documented here, in [contracts/input-schema.md](contracts/input-schema.md) Encapsulation note, and in [validate.tf](terraform/dns/validate.tf) header.

  Record the chosen mechanism (root `terraform_data` precondition) in [modules/dnszones/README.md](modules/dnszones/README.md) (T043).
- [ ] T011 [P] Author [modules/dnszones/locals.tf](modules/dnszones/locals.tf) declaring `local.catalogue` — the 25-entry `map(string)` from spec FR-011 (key → FQDN). Verbatim copy of the FR-011 table.
- [ ] T012 [P] Author [modules/dnszones/outputs.tf](modules/dnszones/outputs.tf) with `zone_ids`, `zone_names`, `resource_group_name`, `resource_group_id`, AND `catalogue_keys` (the SORTED list of catalogue keys — strings only, not FQDNs — exposed so the root stack can size the engine's `services[].count` per T018). Catalogue VALUES (FQDNs) remain INTERNAL per [contracts/output-schema.md](contracts/output-schema.md).
...
- [ ] T013 [P] [US1] Create [terraform/dns/tests/positive_baseline.tftest.hcl](terraform/dns/tests/positive_baseline.tftest.hcl): `command = plan`; reference `var.subscription_id`, `var.region = "swedencentral"`, `var.repo`, `var.topology = "hub"`, `var.tenant = "hub"`, `var.environment = "prd"`, empty `custom_zones`, empty `disable_catalogue_zones`; assert `length(output.zone_ids) == 25`, `output.zone_names["blob"] == "privatelink.blob.core.windows.net"`, `output.resource_group_name == "rg-hub-prd-dns-sdc-001"`, AND `contains(keys(output.naming), output.resource_group_name)` — the `naming` passthrough output exposes the per-stack RG record (FR-023). Use `mock_provider "azurerm"` with matching `subscription_id`.
- [ ] T014 [P] [US1] Create [terraform/dns/tests/determinism_snapshot.tftest.hcl](terraform/dns/tests/determinism_snapshot.tftest.hcl) that asserts `jsonencode({ zone_ids = output.zone_ids, zone_names = output.zone_names }) == file("snapshots/reference.json")` per FR-028 / [research.md § 8](research.md).

### Implementation for User Story 1

- [ ] T015 [US1] Author [modules/dnszones/main.tf](modules/dnszones/main.tf) `azurerm_resource_group.this`. Name + tags read from `var.naming[<rg_canonical>]` where `<rg_canonical>` is computed in `local.rg_canonical_name` using `var.input.purpose` (purpose-aware canonical mirroring `modules/naming` `locals.rg_canonical`). For the prd-hub DNS stack the root stack passes `purpose = "dns"` in `local.input`, yielding `rg-hub-prd-dns-{region_code}-001`. `var.region_code` is supplied by the root stack from `local.region_codes[var.region]` (engine fallback map per T019). Location = `var.region`. Add the `precondition {}` block from T010 here (catalogue-aware unknown-disable-key guard).
- [ ] T016 [US1] Author [modules/dnszones/main.tf](modules/dnszones/main.tf) `module "zone"` instantiating the AVM `Azure/avm-res-network-privatednszone/azurerm` (v0.5.0) wrapper with `for_each = merge({ for k, v in local.catalogue : k => v if !contains(var.disable_catalogue_zones, k) }, { for fqdn in var.custom_zones : fqdn => fqdn })`, `domain_name = each.value`, `parent_id = azurerm_resource_group.this.id`, `tags = local.baseline_tags` (single derivation from `var.input` for catalogue and custom — see plan.md Constitution VIII PASS-WITH-SIMPLIFICATION), and `enable_telemetry = false`. The wrapper owns the underlying `azurerm_private_dns_zone.this` resource; full address from the root stack is `module.dnszones.module.zone[<key>].azurerm_private_dns_zone.this`.
- [ ] T017 [US1] Author [terraform/dns/variables.tf](terraform/dns/variables.tf) declaring **only** typing and the region allowlist for the eight inputs (`subscription_id`, `region`, `repo`, `topology`, `tenant`, `environment`, `custom_zones`, `disable_catalogue_zones` per the rewritten FR-014). Specifically: type declarations + defaults, plus a `validation {}` block on `var.region` enforcing membership in `local.allowed_prd_hub_regions` (declared in T018), plus parse-time validations on `var.topology` / `var.tenant` / `var.environment` (per contracts/input-schema.md). **EXCLUSIVITY**: `var.custom_zones` FQDN-regex validation and de-dup validation are OWNED by T028; `var.disable_catalogue_zones` de-dup validation is OWNED by T033; catalogue-membership validation on `var.disable_catalogue_zones` is OWNED by the module's precondition from T010/T015. Do NOT author any of those in T017.
- [ ] T018 [US1] Author [terraform/dns/locals.tf](terraform/dns/locals.tf) declaring:
  - `local.allowed_prd_hub_regions = ["swedencentral"]` — region allowlist enforced by T017's `var.region` validation.
...
- [ ] T024 [P] [US2] Create [terraform/dns/tests/negative_shadowed_fqdn.tftest.hcl](terraform/dns/tests/negative_shadowed_fqdn.tftest.hcl): `command = plan`, `custom_zones = ["privatelink.blob.core.windows.net"]`, `expect_failures = [terraform_data.guard_custom_zones_no_shadow]` (root precondition from T027 — root-sited rather than module-sited for the Terraform 1.9 reasons documented in [validate.tf](terraform/dns/validate.tf)).
...
- [ ] T027 [US2] Shadowing guard (FR-017) is implemented as `terraform_data.guard_custom_zones_no_shadow` in [terraform/dns/validate.tf](terraform/dns/validate.tf) with a `lifecycle.precondition {}` block consuming `module.dnszones.catalogue_fqdns` (a sorted, diagnostics-only output added to the module so the root guard has a comparison target). Root-sited rather than module-sited for the three Terraform 1.9 reasons documented in the [validate.tf](terraform/dns/validate.tf) header and in [contracts/input-schema.md Encapsulation note](contracts/input-schema.md#encapsulation-note-validatetf-design). Error message names the shadowed FQDN(s) and lists `module.dnszones.catalogue_fqdns` as the protected set. The negative test in T024 targets `expect_failures = [terraform_data.guard_custom_zones_no_shadow]`.
...
- [ ] T031 [P] [US3] Create [terraform/dns/tests/negative_unknown_disable_key.tftest.hcl](terraform/dns/tests/negative_unknown_disable_key.tftest.hcl): `disable_catalogue_zones = ["frobnicate"]`; `expect_failures = [terraform_data.guard_disable_keys_known]` — targeting the root precondition from T010 (root-sited rather than module-sited for the Terraform 1.9 reasons documented in [validate.tf](terraform/dns/validate.tf) and [contracts/input-schema.md Encapsulation note](contracts/input-schema.md#encapsulation-note-validatetf-design)). Test intent: an unknown key MUST hard-fail at plan time with a clear message naming the offending key and listing valid catalogue keys.
...
- [ ] T036 [P] [US4] Create [modules/naming/tests/negative_wrong_topology_private_dns_zone.tftest.hcl](modules/naming/tests/negative_wrong_topology_private_dns_zone.tftest.hcl) (the `topology_scope` check is owned by the naming engine, not by the dnszones stack — the right test home is the engine's own test directory). Drive `var.input = { topology = "spoke", tenant = "sp01", environment = "npd", region = "swedencentral", repo = "x", services = [{ type = "private_dns_zone", count = 1 }] }`; `expect_failures = [check.topology_scope]` (the literal block name from [modules/naming/validate.tf](modules/naming/validate.tf) line 52).

### Implementation for User Story 4

- [ ] T037 [US4] Add [terraform/dns/validate.tf](terraform/dns/validate.tf) `check "subscription_pinned"` block: condition `var.subscription_id == data.azurerm_client_config.current.subscription_id`; error message names both values (FR-029). **Ordering note**: T037 and T038 MUST land in the same PR; within that PR the data source (T038) is the dependency, so author it first. Cross-file order between `validate.tf` and `main.tf` is irrelevant to Terraform.
- [ ] T038 [US4] Add `data "azurerm_client_config" "current" {}` to [terraform/dns/main.tf](terraform/dns/main.tf) for T037 to consume. Author this BEFORE T037 within the same PR (see T037 ordering note).
- [ ] T039 [US4] Author [terraform/dns/moved.tf](terraform/dns/moved.tf) with one `moved {}` block per entry in `specs/002-private-dns-zones/legacy-state-inventory.txt` (T003), mapping each legacy address to its new AVM-wrapped form `module.dnszones.module.zone["<key>"].azurerm_private_dns_zone.this`. Catalogue keys MUST match the FR-011 table; legacy resource names ending in well-known FQDNs map directly to their catalogue key. Example: legacy `module.dns.module.private_dns_zones["privatelink.blob.core.windows.net"].azurerm_private_dns_zone.this` → `module.dnszones.module.zone["blob"].azurerm_private_dns_zone.this`. Note: the file is currently a stub (see header) — the operator populates it from a real backend refresh before cut-over.
...
- [ ] T044 [P] Update [terraform/dns/README.md](terraform/dns/README.md) with: stack purpose, consumer snippet from [quickstart.md § Consuming](quickstart.md#consuming-from-a-spoke-stack), failure-modes table from [quickstart.md](quickstart.md), the snapshot-regen procedure, AND the chosen region-code source per T019 (engine output `module.naming.region_codes` vs the static `local.region_codes` fallback map) so future readers know which path the stack is on.
