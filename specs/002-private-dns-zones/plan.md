...
- `hashicorp/azurerm` `~> 4.0` — `azurerm_resource_group`, `data.azurerm_client_config`. The zone resource itself is supplied by the AVM wrapper below.
- `Azure/avm-res-network-privatednszone/azurerm` `0.5.0` — Azure Verified Module wrapping `azurerm_private_dns_zone`. Adopted per Constitution IX; see Constitution Check IX below.
...
2. `modules/dnszones/` — new thin module owning the day-one catalogue map (the 25 keys → FQDNs) + the per-stack RG + a `for_each` instantiation of the AVM `Azure/avm-res-network-privatednszone/azurerm` wrapper (one module call per zone via `module "zone"`). NO providers block (provider passed implicitly from the root stack).
...
- `azurerm_private_dns_zone.name` (inside the AVM wrapper) MUST be the FQDN literally — Azure requires it. The engine-emitted canonical name (`pdnsz-hub-prd-dns-sdc-001`) is NOT used as the zone name; it is used only for the RG and is also emitted into `naming.names` for audit. This is a documented divergence from Constitution III for zone resources, justified in the Complexity Tracking table.
...
- [x] **II. Minimal, Intent-Only Inputs**: PASS-WITH-NOTE. The stack accepts the **8** inputs enumerated in the rewritten FR-014: 6 required (`subscription_id`, `region`, `repo`, `topology`, `tenant`, `environment`) and 2 optional (`custom_zones`, `disable_catalogue_zones`). The three scope discriminators (`topology`/`tenant`/`environment`) are intent-surface variables matching the repo-wide root-stack input shape (see [terraform/log/variables.tf](../../terraform/log/variables.tf), [terraform/vnet/variables.tf](../../terraform/vnet/variables.tf), [terraform/services/variables.tf](../../terraform/services/variables.tf)) — they are NOT per-resource knobs and they do NOT relax the prd-hub-only constraint (the engine `topology_scope` check on `private_dns_zone` still hard-fails on any non-`(hub, prd)` combination, FR-001). The two optional inputs are catalogue selectors, not per-resource knobs.
...
- [x] **VIII. Tagging Baseline**: PASS-WITH-SIMPLIFICATION. The per-stack `azurerm_resource_group` reads its tags from the engine: `tags = var.naming[<rg_canonical>].tags`. Every zone (catalogue and custom) reads its tags from a single module-internal `local.baseline_tags` map derived from `var.input` (the same 6 baseline keys: `tenant`, `topology`, `environment`, `region`, `managed_by`, `repo`). This is a deliberate simplification of the original "engine tags for catalogue zones, module-internal tags for custom zones" split — the AVM wrapper takes one `tags` argument per `module "zone"` call, and forcing the engine-vs-internal switch per `each.key` complicated the call site without changing the resulting tag set (the engine's baseline-tag derivation already reads from the same `var.input` shape). Override merges, if ever required, are added to `local.baseline_tags` in one place.
...
│   │                        #   + private_dns_zone defaults entry
│   ├── tests/
│   │   └── negative_wrong_topology_private_dns_zone.tftest.hcl  # NEW (T036)
│   └── tests/snapshots/
│       └── reference.json   # REGENERATED in this PR
└── dnszones/                # NEW thin module — owns the catalogue map + RG +
    ├── main.tf              #   AVM `module "zone"` for_each (one per zone)
    ├── variables.tf
...
| AVM wrapper introduces an extra module layer (`module.dnszones.module.zone[<key>]`) between the root stack and the `azurerm_private_dns_zone` resource (Constitution IX). | Constitution IX prefers AVM where a wrapper exists. `Azure/avm-res-network-privatednszone/azurerm` exists and is mature (0.5.0). The wrapper enforces `domain_name` / `parent_id` shape, exposes a telemetry opt-out, and aligns this stack with the rest of the repo migrating to AVM (see `modules/network/`, `modules/loganalytics/`). | "Direct `azurerm_private_dns_zone` resource" — rejected because it bypasses the AVM preference. The extra address layer is purely cosmetic; `for_each` keys and tag semantics remain identical. Migration cost is contained in [terraform/dns/moved.tf](../../terraform/dns/moved.tf) (FR-032). |
