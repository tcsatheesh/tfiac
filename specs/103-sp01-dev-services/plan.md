...
A1. **Selection**: `storage`, `cosmosdb`, `search`, `keyvault`,
    `container_registry`, `app_insights`.
A2. **Private-by-default** (CLAUDE.md mandate): storage PE, search PE,
    keyvault PE — all `true`; cosmosdb is private-only by construction; PEs
    land on the `development` subnet role. `container_registry`'s
    private-endpoint toggle is `false` for this instance.
A3. **Cross-stack wiring**: `vnet_state_backend` → `sp01/npd/vnet.tfstate`
    (PE subnet); `dns_state_backend` → `hub/prd/dns.tfstate`.
A4. **Standalone telemetry**: the `app_insights` component anchors at the
    shared hub Log Analytics workspace (`hub/npd/log.tfstate`) and emits a
    `to-hub-la` diagnostic setting.
