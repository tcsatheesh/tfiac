...
Toggles (private-by-default per CLAUDE.md mandate):
...
Cosmos DB is private-only by construction (engine default); the standalone
`app_insights` component anchors at the shared hub Log Analytics workspace.
...
- Depends on [102-sp01-npd-vnet](../102-sp01-npd-vnet/spec.md) (PE subnet) and
  on the hub DNS stack. Rollout order:
...
- **FR-103-03**: Every selected service that supports Private Link is
  ...
  access, except `container_registry` whose private-endpoint toggle is
...
2. `deploy.yaml` dispatch with `service=services tenant=sp01 environment=dev`
   ...
   spoke vnet exists.
3. Live evidence: storage, search, and keyvault each PNA-Disabled + PE;
