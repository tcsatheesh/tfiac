...
| A4 | — | Standalone `app_insights` anchored at the shared hub LA (`hub/npd/log.tfstate`). | Consistent. |
| A5 | — | Environment=dev (engine rejects npd, FR-025) stated identically. | Consistent. |
| A6 | INFO | Tasks pre-marked `[x]`; instance shipped. | Accepted. |
...
- ✅ Private-by-default mandate satisfied for storage/search/keyvault (PEs)
  and cosmosdb (private-only); `container_registry` PE toggle is `false` for
  this instance.
