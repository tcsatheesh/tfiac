# `/speckit.analyze` — FR-226 dedicated Foundry agent subnet role (engine)
...
|----|----------|---------|------------|
| A-226-01 | MAJOR | Could the agent subnet reuse the existing `container-apps` role instead of adding a new one? | RESOLVED: VC-5 requires the agent subnet to be EXCLUSIVE to one Foundry account and not shared with an ACA managed-environment subnet. A spoke may need both, and the role key doubles as the subnet purpose/name — reuse would collide. C-17 records the distinct-role decision. |
